...
describe Twitter::WebhookSubscribeService do
  ...
  before do
    ...
    allow(twitter_client).to receive_messages(register_webhook: twitter_success_response, unregister_webhook: twitter_success_response,
                                              fetch_subscriptions: instance_double(Twitty::Response, status: '204', body: { message: 'Valid' }), create_subscription: instance_double(Twitty::Response, status: '204', body: { message: 'Valid' }))
  end
  ...
  describe '#perform' do
    ...
    context 'when correct webhook is present' do
      it 'calls create subscription if subscription is not present' do
        allow(twitter_client).to receive_messages(fetch_webhooks: instance_double(Twitty::Response, status: '200',
                                                                                                    body: [{ 'url' => webhook_subscribe_service.send(:twitter_url) }]), fetch_subscriptions: instance_double(Twitty::Response, status: '500'))
        webhook_subscribe_service.perform
